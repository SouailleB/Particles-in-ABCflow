...
random_device gen;   //random number generators, in prevision of multiple particles
...
//point drag_force(point r, quaternion q);  //Force applied on the particle, not used for now
...
point am;
point force, torque;
point force_lab, torque_lab;
quaternion ori;
rigid_body grain;

double qa,qb,qc,qd,norm;
void normalize_quaternion(double y[]){ //Condition that must be imposed at each step
  qa = y[3];
  qb = y[4];
  qc = y[5];
  qd = y[6];
...
void eval_rhs(double time, double y[],double rhs[]){ //evaluation of the derivative of each variables. Used in ode.cpp
...
matrix get_I_bff(){ //Pass I and its inverse to main.cpp
  I.diag3(Ia,Ia,Ib);  
  return I;
}
...
//Torque applied on the spheroid
